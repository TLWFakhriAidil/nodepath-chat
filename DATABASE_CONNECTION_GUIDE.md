...
- `device_setting_nodepath` - Device configuration settings
...
-- Query flow configurations
SELECT id, name, JSON_EXTRACT(nodes, '$[0].data.text') as welcome_text
FROM chatbot_flows_nodepath 
WHERE JSON_EXTRACT(nodes, '$[0].data.text') IS NOT NULL;
...
```sql
-- Delete old flows
DELETE FROM chatbot_flows_nodepath 
...
1. **JSON Support**: MySQL 5.7 introduced native JSON data type support
   ...
   - Used in `chatbot_flows_nodepath.nodes` and `chatbot_flows_nodepath.edges`

...
func main() {
	...
	var tableExists int
	err = db.QueryRow("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'admin_railway' AND table_name = 'users'").Scan(&tableExists)
	if err != nil {
		log.Fatalf("Failed to check table existence: %v", err)
	}

	if tableExists == 0 {
		fmt.Println("❌ users table does NOT exist in the database")
		return
	}

	fmt.Println("✅ users table exists in the database")
	...
	err = db.QueryRow("SHOW CREATE TABLE users").Scan(&tableName, &createStatement)
	if err != nil {
	...
	// Count records
	var recordCount int
	err = db.QueryRow("SELECT COUNT(*) FROM users").Scan(&recordCount)
	if err != nil {
		log.Fatalf("Failed to count records: %v", err)
	}

	fmt.Printf("\n📊 Total records in users: %d\n", recordCount)
}
